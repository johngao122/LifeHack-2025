 ...
 * @param options - The options for the screenshot capture
...
export async function captureActiveTabScreenshot(
    ...
    try {
        ...
        if (!chrome.tabs) {
            const error =
                "Chrome tabs API not available - missing tabs permission or wrong context";
            console.error("[Screenshot] Context error:", error);
            return {
                success: false,
                error: error,
        ...
        if (!chrome.tabs.captureVisibleTab) {
            const error =
                "captureVisibleTab API not available - missing activeTab permission";
        ...
        console.log(
            "[Screenshot] Capturing visible tab with options:",
            captureOptions
        );
        ...
        const dataUrl = await new Promise<string>((resolve, reject) => {
            chrome.tabs.captureVisibleTab(captureOptions, (result) => {
                if (chrome.runtime.lastError) {
                    ...
                    console.log(
                        "[Screenshot] Chrome API success, data URL length:",
                        result.length
                    );
                    resolve(result);
                }
        ...
        console.log(
            "[Screenshot] Successfully extracted base64 data, length:",
            base64Data.length
        );
 ...
 * @param base64Data - The base64 data of the screenshot
 * @param _targetQuality - The target quality of the screenshot
 * @returns The compressed screenshot data
 */
export function compressScreenshotData(
    base64Data: string,
    _targetQuality: number = 0.8
): string {
    return base64Data;
}
 ...
 * @returns True if the screenshot data is valid, false otherwise
 */
 ...
 * @param base64Data - The base64 data of the screenshot
 * @returns The estimated size of the screenshot in bytes
 */
