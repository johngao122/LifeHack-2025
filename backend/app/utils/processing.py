...
class ProductProcessor:
    def transform_single_product(self, product: Dict[str, Any]) -> Optional[Dict[str, Any]]: 
        ...
        environmental_score_data = self.transform_environmental_data(product)

        categories_hierarchy = product.get("categories_hierarchy", [])
        categories = self.transform_categories_hierarchy(categories_hierarchy)
        logger.debug(
            f"Categories processed: {len(categories)} English categories from hierarchy"
        )

        labels_raw = product.get("labels", "")
        labels = self.transform_labels(labels_raw)
    ...
    def transform_environmental_data(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        material_scores = self.transform_material_scores(ecoscore_data)
        if material_scores:
    ...
    def transform_material_scores(self, ecoscore_data: Dict[str, Any]) -> Dict[str, Any]:
        material_scores = {}
    ...
    def transform_labels(self, labels_str: str) -> List[str]:
    ...
    def transform_categories_hierarchy(self, categories_hierarchy: List[str]) -> List[str]:
    ...
    def select_best_product(self, products: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        if not products:
    ...
    def calculate_product_quality_score(self, product: Dict[str, Any]) -> float:
        score = 0.0
